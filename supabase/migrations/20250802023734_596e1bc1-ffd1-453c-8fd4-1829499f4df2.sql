-- Fix column ambiguity issue in the adaptive questions function
...
BEGIN
    ...
    weak_topic_questions AS (
        SELECT DISTINCT ON (c.subtopic) to_jsonb(c.*) as question_json
        FROM curriculum c
    ...
    review_questions AS (
        SELECT to_jsonb(c.*) as question_json
        FROM curriculum c
    ...
    SELECT question_json FROM weak_topic_questions
    UNION ALL
    SELECT question_json FROM review_questions
    UNION ALL
    SELECT question_json FROM new_questions
    UNION ALL
    SELECT question_json FROM mastery_questions;
END;
$function$;