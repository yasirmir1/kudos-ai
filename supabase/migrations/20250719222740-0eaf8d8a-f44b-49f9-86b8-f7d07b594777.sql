-- Update the age_group enum to use school years instead of age ranges
ALTER TYPE age_group RENAME TO age_group_old;

CREATE TYPE age_group AS ENUM ('year 2-3', 'year 4-5', '11+');

-- Update all tables that use the age_group enum
ALTER TABLE student_profiles 
  ALTER COLUMN age_group DROP DEFAULT,
  ALTER COLUMN age_group TYPE age_group USING 
    CASE age_group_old::text
      WHEN '6-7' THEN 'year 2-3'::age_group
      WHEN '8-9' THEN 'year 4-5'::age_group
      WHEN '10-11' THEN '11+'::age_group
    END,
  ALTER COLUMN age_group SET DEFAULT 'year 4-5'::age_group;
...
-- Update the enhanced adaptive questions function
