...
export const TopicItem: React.FC<TopicItemProps> = ({
  ...
  return <div className={`px-4 py-4 rounded-lg border transition-all duration-200 ${isClickable ? 'cursor-pointer hover:bg-muted/50 hover:border-primary/50' : 'bg-muted/20'}`} onClick={onClick}>
      <div className="flex items-center justify-between gap-6">
        ...
        <div className="flex-1 min-w-0">
          ...
          {showClickHint && <span className="text-sm text-primary">Click to practice</span>}
        </div>
        
        {/* Circular progress and count with increased spacing */}
        <div className="flex items-center gap-6 flex-shrink-0 pr-2">
          <CircularProgress value={accuracyPercent} size={48} color={getProgressColor()} />
